...
import configs.real_11322 as config_file
...
projections_alligned = np.float32(
    mrcfile.open(os.path.join(config.path_load, config.name_best_proj), permissive=True).data)
import tomopy
def TV_tomopy(projections, angles, reg_tv, nit_tv, n3):
    ...
    return recon[:, :, ::-1]
angles = np.linspace(config.view_angle_min,config.view_angle_max,config.Nangles)
V_no_deformed_tv = TV_tomopy(projections_alligned, angles, config.lamb_tv, config.nit_tv, projections_alligned.shape[1]//2)
tmp = V_no_deformed_tv
tmp = (tmp - tmp.min()) / (tmp.max() - tmp.min())
tmp = np.clip(tmp, a_min=np.quantile(tmp, 0.005), a_max=np.quantile(tmp, 0.995))
display_XYZ(tmp, name="TV_alligned")

out = mrcfile.new(os.path.join(config.path_save_data, 'evaluation',
                               "volumes", "TV_alligned.mrc"), np.moveaxis(V_no_deformed_tv.astype(np.float32), 2, 0),
                  overwrite=True)
out.close()
