...
path_save = "./results/"+str(volume_name)+"_SNR_"+str(SNR_value)+"_size_"+str(n1)+"_no_PSF/"
...
if os.path.exists("/raid/Valentin/"+str(volume_name)+"/"+name_volume):
    path_volume = "/raid/Valentin/"+str(volume_name)+"/"+name_volume
elif os.path.exists("/users/staff/dmi-dmi/debarn0000/data_nobackup/"+str(volume_name)+"/"+name_volume):
    path_volume = "/users/staff/dmi-dmi/debarn0000/data_nobackup/"+str(volume_name)+"/"+name_volume
elif os.path.exists("/home/debarn0000/Documents/Data/shrec2021_full_dataset/"+str(volume_name)+"/"+name_volume):
    path_volume = "/home/debarn0000/Documents/Data/shrec2021_full_dataset/"+str(volume_name)+"/"+name_volume
else:
...
V = V[nv[0]//2-n3_patch//2:nv[0]//2+n3_patch//2,nv[1]//2-n1_patch//2:nv[1]//2+n1_patch//2,nv[2]//2-n2_patch//2:nv[2]//2+n2_patch//2]
V = resize(V,(n3,n1,n2))
V = np.swapaxes(V,0,1)
V = np.swapaxes(V,1,2)
...
angles = np.linspace(view_angle_min,view_angle_max,Nangles)
...
# TODO: implement spatially varying PSF
...
if sigma_PSF!=0:
    ...
    PSF_ext[n1//2-supp//2:n1//2+supp//2,n2//2-supp//2:n2//2+supp//2] = G[n1//2-supp//2:n1//2+supp//2,n2//2-supp//2:n2//2+supp//2]
    PSF_ext_t = torch.tensor(PSF_ext).type(torch_type).to(device)
    PSF_ext_fft_t = torch.fft.fft2(PSF_ext_t).view(1,n1,n2)
else: 
    PSF = 0
...
affine_tr = []
local_tr = []
for i in range(Nangles*number_sub_projections):
    scaleX, scaleY, shiftX, shiftY, shearX, shearY, angle  = utils_deformation.generate_params_deformation(scale_min,scale_max,shift_min,shift_max,shear_min,shear_max,angle_min,angle_max)
    affine_tr.append(utils_deformation.AffineTransform(scaleX, scaleY, shiftX, shiftY, shearX, shearY, angle ).cuda())

    depl_ctr_pts = torch.randn(2,N_ctrl_pts_local_def[0],N_ctrl_pts_local_def[1]).to(device).type(torch_type)
    depl_ctr_pts[0] = depl_ctr_pts[0]/n1*sigma_local_def
    depl_ctr_pts[1] = depl_ctr_pts[1]/n2*sigma_local_def
    ...
    if i < 10:
        nsr = (n1*4,n2*4)
        Nsp = (n1//20,n2//20) # number of Diracs in each direction
        supp = n1//70

        # Display local deformations
        utils_display.display_local(field,field_true=None,Npts=Nsp,img_path=path_save+"deformations/local_quiver_"+str(i),
                                    img_type='.png',scale=1,alpha=0.8,width=0.0015,wx=n1//2,wy=n2//2)
        ...
        imageio.imwrite(path_save+"deformations/global_deformation_indiviudal_"+str(i)+".png",tmp)
    

projections_clean = operator_ET(V_t)
projections_clean = projections_clean[:,None].repeat(1,number_sub_projections,1,1).reshape(-1,n1,n2)
...
np.save(path_save+"global_deformations.npy",affine_tr)
np.save(path_save+"local_deformations.npy",local_tr)
np.savez(path_save+"volume_and_projections.npz",projections_noisy=projections_noisy.detach().cpu().numpy(),projections_deformed=projections_deformed.detach().cpu().numpy(),projections_deformed_global=projections_deformed_global.detach().cpu().numpy(),projections_clean=projections_clean.detach().cpu().numpy(),PSF=PSF)
np.savez(path_save+"parameters.npz",Nangles=Nangles,angle_min=angle_min,angle_max=angle_max, n1=n1,n2=n2,n3=n3,SNR_value=SNR_value,sigma_PSF=sigma_PSF)
...
for k in range(Nangles):
    ...
    tmp = (tmp - tmp.max())/(tmp.max()-tmp.min())
    tmp = np.floor(255*tmp).astype(np.uint8)
    imageio.imwrite(path_save+"projections/clean_"+str(k)+".png",tmp)

    tmp = projections_deformed[k].detach().cpu().numpy()
    tmp = (tmp - tmp.max())/(tmp.max()-tmp.min())
    tmp = np.floor(255*tmp).astype(np.uint8)
    imageio.imwrite(path_save+"projections/deformed_"+str(k)+".png",tmp)

    tmp = projections_noisy[k].detach().cpu().numpy()
    tmp = (tmp - tmp.max())/(tmp.max()-tmp.min())
    tmp = np.floor(255*tmp).astype(np.uint8)
    imageio.imwrite(path_save+"projections/noisy_"+str(k)+".png",tmp)


projections_noisy_avg = projections_noisy.reshape(Nangles,-1,n1,n2).mean(1)
projections_noisy_no_deformed_avg =  projections_noisy_no_deformed.reshape(Nangles,-1,n1,n2).mean(1)
...
out = mrcfile.new(path_save+"V_FBP.mrc",np.moveaxis(V_FBP.detach().cpu().numpy().reshape(n1,n2,n3),2,0),overwrite=True)
out.close() 
out = mrcfile.new(path_save+"V_FBP_no_deformed.mrc",np.moveaxis(V_FBP_no_deformed.detach().cpu().numpy().reshape(n1,n2,n3),2,0),overwrite=True)
out.close() 
out = mrcfile.new(path_save+"V.mrc",np.moveaxis(V_t.detach().cpu().numpy().reshape(n1,n2,n3),2,0),overwrite=True)
out.close() 

out = mrcfile.new(path_save+"projections.mrc",projections_noisy.detach().cpu().numpy(),overwrite=True)
out.close() 
...
out = mrcfile.new(path_save+"projections_reversed.mrc",projections_noisy_reversed.detach().cpu().numpy(),overwrite=True)
out.close()
out = mrcfile.new(path_save+"projections_noisy_no_deformed.mrc",projections_noisy_no_deformed.detach().cpu().numpy(),overwrite=True)
out.close()
out = mrcfile.new(path_save+"projections_noisy_no_deformed_reversed.mrc",projections_noisy_no_deformed_reversed.detach().cpu().numpy(),overwrite=True)
out.close()