...
def get_default_realData():
    ...
    config.fixed_angle = -4.4 # in degrees
    config.use_gains = True
...
def get_default_realData_multiresolution():
    config = ml_collections.ConfigDict()
    # simulation
    # config = simulation = ml_collections.ConfigDict()
    ...
    config.multires_params.startResolution = 8
    config.multires_params.upsample = False
    config.multires_params.batch_size = [20,10,5,4,3,2,1] # number of viewing direction per iteration
    config.multires_params.n_rays =  [50,100,200,400,800,1000,1500] # number of sampling rays per viewing direction
    config.multires_params.ray_length = [50,100,200,400,1000,1000,1200,1500] #int(np.floor(n1*z_max))
    config.multires_params.ray_change_epoch = [100,300,400,800,1000,1200,1500,2000]

    # # Parameters for the data generation
    config.path_save_data = "./results/"+str(config.volume_name)+"_size_"+str(config.n1)+"_"+str(config.n2)+"_multi_res/"
    config.path_save = "./results/"+str(config.volume_name)+"_size_"+str(config.n1)+"_"+str(config.n2)+"_multi_res/"
